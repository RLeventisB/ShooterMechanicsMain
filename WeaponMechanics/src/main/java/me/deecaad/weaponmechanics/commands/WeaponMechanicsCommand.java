...
public class WeaponMechanicsCommand
{
    ...
    public static final HashMap<UUID, Integer> lastGroundTime = new HashMap();
    ...
    public static void build()
    {
        ...
        CommandBuilder command = new CommandBuilder("wm")
            ...
            .withSubcommand(new CommandBuilder("give")
                .withPermission("weaponmechanics.commands.give").withDescription("Gives the target(s) with requested weapon(s)")
            ...
            .withSubcommand(new CommandBuilder("updateground")
                ...
                .executes(CommandExecutor.any((sender, args) ->
                {
                    for (Entity entity : (List<Entity>) args[0])
                    {
                        ...
                        if (!hasRegistredBack)
                        {
                            if (!lastGroundTime.containsKey(plrUuid))
                            {
                                lastGroundTime.put(plrUuid, 0);
                                player.sendMessage(RED + "Hola!!! tienes 20 segundos para salir de tu spawn\no si no pasaran cosas feas.");
                            }
                            else
                            {
                                int time = lastGroundTime.get(plrUuid);

                                if (time >= 400)
                                {
                                    EntityDamageEvent event = new EntityDamageEvent(player, EntityDamageEvent.DamageCause.KILL, Double.MAX_VALUE);
                                    ...
                                    lastGroundTime.remove(plrUuid);
                                    continue;
                                }
                                else if (time % 20 == 0 && time >= 200)
                                {
                                    player.sendMessage(RED + "Tienes " + (20 - time / 20) + " segundos para salir de tu spawn >:(");
                                    player.playSound(player.getLocation(), Sound.UI_BUTTON_CLICK, 1, 1);
                                }
                                time++;
                                lastGroundTime.put(plrUuid, time);
                            }
                        }
                        Location plrLoc = player.getLocation();
                        boolean isOnSpawnArea = false;
                        for (double i = -0.3; i < 0.31 && !isOnSpawnArea; i += 0.3)
                        {
                            for (double j = -0.3; j < 1.81 && !isOnSpawnArea; j += 0.3)
                            {
                                for (double k = -0.3; k < 0.31 && !isOnSpawnArea; k += 0.3)
                                {
                        ...
                        if (isOnSpawnArea)
                        {
                            ...
                            if (plrLoc.getWorld().getBlockAt(lastGrounded).getType() == Material.STRUCTURE_VOID)
                                continue;
                            player.teleport(lastGrounded);
                            player.sendMessage(ChatColor.RED + "oops entraste a una zona restringida!!! te devolveremos para tu seguridad.\n(seras mutilado si sigues aqui)");
                        ...
                        else
                        {
                            if (!entity.isOnGround())
                                continue;
                            if (plrLoc.getBlock().isEmpty())
                            {
                                if (!hasRegistredBack)
                                {
                                    player.sendMessage(DARK_GREEN + "Haz salido de tu spawn!!!! no podras volver desde ahora >:(");
                                }
            ...
            .withSubcommand(new CommandBuilder("removeground")
                ...
                .withArgument(new Argument<>("target", new EntityListArgumentType()).withDesc("Quien procesaras."))
                .withArgument(new Argument<>("extratime", new DoubleArgumentType(0, Double.MAX_VALUE), 0.0).withDesc("cuanto tiempo extra habra en salir del spawn"))
                .executes(CommandExecutor.any((sender, args) ->
                {
                    for (Entity entity : (List<Entity>) args[0])
                    {
                        if (entity instanceof Player player)
                        {
                            ...
                            {
                                extraTime = (Double) args[1] * -20;
                            }
                            lastGroundTime.put(plrUuid, (int) (0 + extraTime));
                            player.sendMessage(RED + "Hola!!! tienes " + (20 + extraTime / -20) + " segundos para salir de tu spawn\no si no pasaran cosas feas.");
                        ...
                        for (int i = -30; i < 31; ++i)
                        {
                            for (int j = -30; j < 31; ++j)
                            {
                                for (int k = -30; k < 31; ++k)
                                {
                                    ...
                                    cloneLoc.add(i, j, k);
                                    if (cloneLoc.getBlock().getType() != Material.STRUCTURE_VOID)
                                        continue;
        ...
        new BukkitRunnable()
        {
            ...
            public void run()
            {
                ...
                for (Entity entity : targets)
                {
                    ...
                    if (NumberUtil.equals(sumOf, 0.0))
